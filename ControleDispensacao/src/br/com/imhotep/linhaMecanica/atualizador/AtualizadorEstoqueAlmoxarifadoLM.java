...
public class AtualizadorEstoqueAlmoxarifadoLM extends GerenciadorMecanico{
	...
	public void lockEstoque(int idEstoqueAlmoxarifado, String lote, int cont) throws ExcecaoEstoqueLockAcimaUmMinuto, ExcecaoEstoqueLock {
		try {
			...
			while (rs.next()) { 
				...
				if(lock){
					...
					if(minutos >= 1){
						...
						if(cont < 5)
							lockEstoque(idEstoqueAlmoxarifado, lote, cont++);
						else
