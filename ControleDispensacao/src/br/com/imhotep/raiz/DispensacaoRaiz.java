...
//		aeh.setMaterial(prescricaoItemEstoqueSaida.getEstoque().getMaterial());
	...
	public void carregaEstoqueEdicao(){
		...
		if(getListPrescricaoItemEstoqueSaida() == null){
			FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR,"N�o foi encontrado a prescri��o.", ""));
		}
	}
		...
		if(prescricaoItem.getQuantidadeLiberada() != null){
			if(liberaDose(prescricaoItem)){
				try{
					...
					String msg = "Dispensa��o realizada com sucesso.";
					finalizaTransacao(msg);
				}catch(Exception e){
					String msg = "Ocorreu um erro ao fazer a dispensa��o.";
					catchTransacao(msg, e);
				}finally{
					finallyTransacao();
				}
