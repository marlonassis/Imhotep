...
@ManagedBean
@RequestScoped
public class EstornoMovimentacaoFarmaciaConsultaRaiz  extends ConsultaGeral<DevolucaoMedicamento>{

	public boolean isDiferecaNegativa(MovimentoLivro item){
		int idE = item.getEstoque().getIdEstoque();
		int idML = item.getIdMovimentoLivro();
		String sql = "select coalesce((select sum(a.in_quantidade_movimentacao) from tb_movimento_livro a "+
						"inner join tb_tipo_movimento b on a.id_tipo_movimento = b.id_tipo_movimento "+
						"where b.tp_operacao = 'E' and a.id_estoque = "+idE+" and a.id_movimento_livro != "+idML+"), 0) "+
						"- "+
	...
	public boolean isPrimeiroMovimento(MovimentoLivro item) throws ExcecaoEstornoFalha{
		int idEstoque = item.getEstoque().getIdEstoque();
		String sql = "select a.id_movimento_livro = "+item.getIdMovimentoLivro()+
					 " primeiroMovimento from tb_movimento_livro a where "+
					 "a.dt_data_movimento =  "+
	...
	public boolean isTemMovimentoPosteriorSaida(MovimentoLivro item) throws ExcecaoEstornoFalha{
		int idEstoque = item.getEstoque().getIdEstoque();
		String dataFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(item.getDataMovimento());
		String sql = "select count(a.id_movimento_livro) qtdMovimento from tb_movimento_livro a " +
					 "inner join tb_tipo_movimento b on a.id_tipo_movimento = b.id_tipo_movimento "+
	...
	public boolean isSegundoMovimentoSaida(MovimentoLivro item) throws ExcecaoEstornoFalha{
		int idEstoque = item.getEstoque().getIdEstoque();
		String dataFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(item.getDataMovimento());
		String sql = "select b.tp_operacao != 'E' saida from tb_movimento_livro a "+ 
					 "inner join tb_tipo_movimento b on a.id_tipo_movimento = b.id_tipo_movimento "+
					 "where "+
